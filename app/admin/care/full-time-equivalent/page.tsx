...
export default async function CareFullTimeEquivalentPage({ searchParams }: { searchParams: { ym?: string } }) {
  ...
  for (const user of users) {
    ...
    const current = rowsByJobType.get(jobType) ?? { jobType, count: 0, fullTimeCount: 0, monthlyMinutes: 0, monthlyScheduledMinutes: 0 };
    current.count += 1;
  ...
  const rows = Array.from(rowsByJobType.values())
    .map((row) => ({ ...row, fte: standardMonthlyMinutes > 0 ? row.monthlyMinutes / standardMonthlyMinutes : 0 }))
        ...
        <header className="sticky top-0 z-10 border-b bg-white/90 px-5 py-4 backdrop-blur">
          <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">シフト予定時間から職種別の常勤換算を概算します。</p>
            </div>
            <div className="flex items-center gap-2">
              <Link href={`/admin/care/full-time-equivalent?ym=${buildMonthNav(ym, -1)}`} className="rounded-xl border bg-white px-3 py-2 font-black text-slate-700">前月</Link>
              <div className="rounded-xl border bg-white px-4 py-2 font-black text-slate-900">{monthLabel(year, month)}</div>
              <Link href={`/admin/care/full-time-equivalent?ym=${buildMonthNav(ym, 1)}`} className="rounded-xl border bg-white px-3 py-2 font-black text-slate-700">翌月</Link>
              <Link href="/admin/care/full-time-equivalent/settings" className="rounded-xl bg-slate-900 px-4 py-2 font-black text-white">基準時間設定</Link>
            </div>
          </div>
        </header>
        ...
        <div className="mx-auto max-w-7xl px-5 py-6">
          ...
          {!rule && (
            <div className="mb-5 rounded-3xl border border-orange-200 bg-orange-50 p-5 text-sm font-bold text-orange-800">
              常勤換算の基準時間が未設定です。初期値として160時間を使用しています。
            </div>
          )}
          ...
          <section className="overflow-hidden rounded-3xl bg-white shadow-sm">
            <div className="border-b p-5">
              ...
              <p className="text-sm text-slate-500">{daysInMonth(year, month)}日分のシフト予定から概算しています。</p>
            </div>
                ...
                <tbody>
                  {rows.map((row) => (
                    <tr key={row.jobType} className="border-t">
                      ...
                      <td className="p-4 font-bold text-slate-600">{row.monthlyScheduledMinutes > 0 ? minutesToHHMM(row.monthlyScheduledMinutes) : "未設定"}</td>
                      ...
                      <td className="p-4">
                        <span className={`rounded-full px-3 py-1 text-xs font-black ${row.monthlyMinutes > 0 ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-500"}`}>
                          {row.monthlyMinutes > 0 ? "算出済み" : "勤務予定なし"}
                        </span>
                      </td>
                    </tr>
                  ))}
                  {rows.length === 0 && (
                    <tr>
                      <td colSpan={7} className="p-8 text-center font-bold text-slate-500">対象スタッフがまだ登録されていません。</td>
                    </tr>
                  )}
                </tbody>
              </table>
