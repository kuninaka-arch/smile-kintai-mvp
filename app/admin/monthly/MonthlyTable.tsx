...
export async function MonthlyTable({ companyId, ym, selectedDepartment, requestedPage }: MonthlyTableProps) {
  ...
  console.log("[PERF][admin-monthly-table]", perfId, "total:start");
  ...
  console.log("[PERF][admin-monthly-table]", perfId, "build-user-where", Date.now() - userWhereStart, "ms");

  const companyStart = Date.now();
  const periodPromise = getPeriodLock(companyId, ym).then((result) => {
    console.log("[PERF][admin-monthly-table]", perfId, "load-company", Date.now() - companyStart, "ms");
    return result;
  });

  const userCountStart = Date.now();
  const totalCountPromise = prisma.user.count({ where: userWhere }).then((count) => {
    console.log("[PERF][admin-monthly-table]", perfId, "load-user-count", Date.now() - userCountStart, "ms");
    return count;
  });
  ...
  const [attendanceLogs, shifts, paidLeaveSummaries, leaveRequests] = userIds.length > 0
    ? await Promise.all([
        (async () => {
          ...
          console.log("[PERF][admin-monthly-table]", perfId, "monthly-related-attendance-logs", Date.now() - startedAt, "ms");
          return result;
        })(),
        (async () => {
          const startedAt = Date.now();
          ...
          console.log("[PERF][admin-monthly-table]", perfId, "monthly-related-shifts", Date.now() - startedAt, "ms");
          return result;
        })(),
        (async () => {
          const startedAt = Date.now();
          ...
          console.log("[PERF][admin-monthly-table]", perfId, "monthly-related-paid-leaves", Date.now() - startedAt, "ms");
          return result;
        })(),
        (async () => {
          const startedAt = Date.now();
          ...
          console.log("[PERF][admin-monthly-table]", perfId, "monthly-related-leave-requests", Date.now() - startedAt, "ms");
          return result;
        })()
      ])
    : [[], [], [], []];
  console.log("[PERF][admin-monthly-table]", perfId, "load-monthly-related-data", Date.now() - relatedStart, "ms");
  ...
  console.log("[PERF][admin-monthly-table]", perfId, "map-monthly-related-data", Date.now() - mapStart, "ms");
  ...
  console.log("[PERF][admin-monthly-table]", perfId, "aggregate-monthly-data", Date.now() - aggregateStart, "ms");
  ...
  console.log("[PERF][admin-monthly-table]", perfId, "render-prep", Date.now() - renderPrepStart, "ms");
  console.log("[PERF][admin-monthly-table]", perfId, "total", Date.now() - totalStart, "ms");
