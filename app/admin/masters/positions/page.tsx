...
export default async function PositionsMasterPage() {
  ...
  const items = await prisma.positionMaster.findMany({
      ...
      <section className="lg:ml-64">
        <header className="sticky top-0 z-10 border-b bg-white/90 px-5 py-4 backdrop-blur">
          <div className="mx-auto max-w-7xl">
            ...
            <h1 className="mt-2 text-2xl font-black">役職マスタ</h1>
            <p className="text-sm text-slate-500">社員に付与する役職を管理します。</p>
          </div>
        </header>
        ...
        <div className="mx-auto grid max-w-7xl gap-6 px-5 py-6 xl:grid-cols-[420px_1fr]">
          <section className="rounded-3xl bg-white p-5 shadow-sm">
            ...
            <MasterForm kind="position" mode="create" />
          </section>
                ...
                <tbody>
                  {items.map((item) => (
                    <tr key={item.id} className="border-t">
                      ...
                      <td className="p-4">
                        <MasterForm kind="position" mode="edit" item={{ id: item.id, code: item.code, name: item.name, sortOrder: item.sortOrder, isActive: item.isActive }} />
                      </td>
                    </tr>
                  ))}
                </tbody>
