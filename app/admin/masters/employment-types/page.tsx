      ...
      <section className="lg:ml-64">
        <header className="sticky top-0 z-10 border-b bg-white/90 px-5 py-4 backdrop-blur">
          <div className="mx-auto max-w-7xl">
            ...
            <h1 className="mt-2 text-2xl font-black">雇用区分マスタ</h1>
            <p className="text-sm text-slate-500">正社員・契約社員・パート・アルバイトなどを管理します。</p>
          </div>
        </header>
        ...
        <div className="mx-auto grid max-w-7xl gap-6 px-5 py-6 xl:grid-cols-[420px_1fr]">
          <section className="rounded-3xl bg-white p-5 shadow-sm">
            ...
            <MasterForm kind="employmentType" mode="create" />
          </section>
                ...
                <tbody>
                  {items.map((item) => (
                    <tr key={item.id} className="border-t">
                      ...
                      <td className="p-4">
                        <MasterForm kind="employmentType" mode="edit" item={{ id: item.id, code: item.code, name: item.name,  sortOrder: item.sortOrder, isActive: item.isActive }} />
                      </td>
                    </tr>
                  ))}
                </tbody>
